# Claude Code Orchestra Integration Plan v16
...
_ALLOWED_BASE_DIRS = _build_allowed_dirs()
...
def guard_context(content: str, source_files: list[str] = None) -> str:
    ...
    Args:
        ...
        source_files: List of source file paths. If None, treated as "unknown origin"
                      and consent policy determines behavior.
    ...
    # Step 2: Unknown origin policy (v14 D-1, v16 F-1: treat [] same as None)
    ...
    if not source_files:  # None or empty list
        ...
        # For "block" and "redact" policies, proceed with content-only scanning
        _audit_log("unknown_origin", f"No source_files provided, policy={policy}")

    # Step 3: Enforce allowed base directories (v13 C-1, v16 F-1: explicit None check)
...
v15 Codex Verify flagged 4 issues (1 high + 2 medium + 1 low). This v16 addresses all of them:

1. **[High] Empty source_files bypass** (F-1): Changed `if source_files:` to `if source_files is not None and len(source_files) > 0:` for allowlist/blocked-file checks. Unknown-origin handling now triggers on `not source_files` (catches both None and []).
2. **[Medium] Double active_calls decrement** (F-2): Removed `active_calls` decrement from `record_call()`. Only `release_slot()` decrements concurrency counter.
3. **[Medium] CLAUDE_PROJECT_DIR not guaranteed** (F-3): `bootstrap.py` sets `CLAUDE_PROJECT_DIR` from settings or auto-detection. `_build_allowed_dirs()` emits audit warning if no project dir env var is configured.
4. **[Low] CONSENT_POLICY import-time only** (F-4): Replaced module-level `CONSENT_POLICY` with `_get_consent_policy()` called inside `guard_context()` on each invocation. Runtime env changes take effect immediately.

All v11-v15 fixes are preserved. Please review and verify these v16 fixes are adequate.
