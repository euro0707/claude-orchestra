# Claude Code Orchestra Integration Plan v12
...
def guard_context(content: str, source_files: list[str] = None) -> str:
    ...
    Steps:
    1. Check file allowlist (if source files provided)
    2. Scan for secrets
    3. Redact any found secrets
    ...
    Raises:
        ContextGuardError: If content is blocked (e.g., entire file is a secret)
    """
    # Step 1: File allowlist check
    if source_files:
    ...
    # Step 3: Redact if secrets found
    if findings:
...
def _save_state(state: dict) -> None:
    """Persist budget state (best-effort)."""
    try:
...
```python
from context_guard import guard_context
...
def call_codex_safe(mode: str, context: str, timeout: int = 300) -> dict:
    ...
    try:
        ...
        # Estimate tokens from output size (rough: 4 chars per token)
...
+-- context_guard.py       # [NEW] Secret scanning and redaction
+-- resilience.py          # [NEW] Retry, backoff, failure classification
+-- output_schemas.py      # [NEW] Output validation schemas
+-- env_check.py           # [NEW] Environment prerequisites check
+-- budget.py              # [NEW] Token budget and concurrency control
```
...
v11 Codex Review flagged 7 issues. This v12 addresses all of them:

1. **[Critical] External transmission security**: New `context_guard.py` with 4-layer protection
2. **[High] CLI injection**: Confirmed all calls use shell=False + arg arrays (audit in v12)
3. **[High] Error handling**: New `resilience.py` with retry/backoff/fallback
4. **[Medium] Output structure**: New `output_schemas.py` + validation in wrappers
5. **[Medium] Storage versioning**: New docs JSONL index + task-scoped dirs in vault_sync
6. **[Medium] Environment check**: New `env_check.py` with capability matrix
7. **[Medium] Cost control**: New `budget.py` with token budget + concurrency limit

Plus 4 medium issues from verify_result (B-1 through B-4) and 1 low (B-5).

Please review and verify these fixes are adequate.
