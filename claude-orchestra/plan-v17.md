# Claude Code Orchestra Integration Plan v17
...
previous_reviews:
  ...
  - "v13: NOT APPROVED, confidence: 7, 1 high + 3 medium + 2 low (Codex CLI verify)"
  - "v14: NOT APPROVED, confidence: 7, 1 high + 2 medium + 2 low (Codex CLI verify)"
  - "v15: NOT APPROVED, confidence: 7, 1 high + 2 medium + 1 low (Codex CLI verify)"
  - "v16: APPROVED, confidence: 7, 0 high + 2 medium + 2 low (Codex CLI verify)"
  - "v17: APPROVED, confidence: 7, 0 high + 3 medium + 1 low (Codex CLI verify)"
---
...
## v17 Codex Verify Review H: Issues (3 medium + 1 low)

| # | Severity | v17 Issue | v17 Response |
|---|----------|----------|--------------|
| H-1 | medium | Default `redact` policy lets unknown-origin content (`source_files=None/[]`) bypass allowlist/blocklist checks — proprietary code can leak | **Design tightening**: Add `ORCHESTRA_STRICT_ORIGIN=1` env var. When set, `guard_context()` blocks unknown-origin content even under `redact` policy. Default remains `redact` (permissive) for backwards compatibility, but hooks/production should set strict mode. |
| H-2 | medium | `_ALLOWED_BASE_DIRS` computed at import time, stale if env changes | **Already fixed in v17 G-1** — `_build_allowed_dirs()` is called per-invocation. Codex may have missed this change in the plan text. No additional fix needed. |
| H-3 | medium | `check_budget` does not catch lock/I/O failures, exceptions bubble up | **Fix**: Wrap `_load_state_locked()` call in `check_budget()` with `try/except`, return permissive fallback `{"allowed": True, ...}` on failure. Consistent with "best-effort" design promise. |
| H-4 | low | Security policy text says only `~/.claude` allowed, but implementation allows `CLAUDE_PROJECT_DIR` + `ORCHESTRA_ALLOWED_DIRS` | **Already fixed in v17 G-3** — `docs/security-policy.md` documents the expanded allowlist. No additional fix needed. |

### Review I: v17 Codex Verify re-review (3 medium + 2 low)

| # | Severity | Issue | Response |
|---|----------|-------|----------|
| I-1 | medium | `_ALLOWED_BASE_DIRS` import-time cache | **Already fixed in G-1** (Codex read stale context) |
| I-2 | medium | budget non-blocking lock exception | **Already fixed in G-2 + H-3** (Codex read stale context) |
| I-3 | medium | `fallback_to_orchestrator()` passes unredacted `original_task` → secret leakage | **Fix below**: Apply `redact_secrets()` before including in fallback |
| I-4 | low | Security policy text mismatch | **Already fixed in G-3** |
| I-5 | low | Test coverage for empty `source_files` + runtime policy changes | Addressed in implementation test plan |

### H-1 Fix: Strict origin mode

In `context_guard.py`, add `ORCHESTRA_STRICT_ORIGIN` check in the unknown-origin handling:

```python
    # Step 2: Unknown origin policy (v14 D-1, v16 F-1, v17 H-1)
...
Also add to `docs/security-policy.md`:

```markdown
### Strict Origin Mode (`ORCHESTRA_STRICT_ORIGIN`)

When `ORCHESTRA_STRICT_ORIGIN=1` is set, `guard_context()` blocks any content
where `source_files` is not provided, regardless of consent policy. This prevents
accidentally sending proprietary code without explicit provenance tracking.

Recommended for production/CI environments:
```bash
export ORCHESTRA_STRICT_ORIGIN=1
```
```

### H-3 Fix: Budget check permissive fallback

In `budget.py`, wrap the `check_budget` implementation:
...
## v17 Changes (from v16 remaining items: 2 medium + 2 low)

### G-1 (medium): `_ALLOWED_BASE_DIRS` cached at import time

**Problem**: `_ALLOWED_BASE_DIRS = _build_allowed_dirs()` is computed once at module load. If `CLAUDE_PROJECT_DIR` or `ORCHESTRA_ALLOWED_DIRS` are set after import (e.g., by `bootstrap.py`), the guard operates on stale data.

**Fix**: Remove the module-level `_ALLOWED_BASE_DIRS` constant. Replace `_build_allowed_dirs()` calls inline in `enforce_allowed_dirs()` and `guard_context()` so directories are resolved per-call.

```python
# REMOVED: _ALLOWED_BASE_DIRS = _build_allowed_dirs()
...
def enforce_allowed_dirs(source_files: list[str]) -> list[str]:
    ...
    v17: Directories are now resolved per-call (not cached at import time).
    """
    allowed_dirs = _build_allowed_dirs()
    violations = []
...
In `guard_context()`, the error message also uses `_build_allowed_dirs()` instead of the cached list:

```python
    # Step 3: Enforce allowed base directories (v13 C-1, v16 F-1, v17 G-1)
...
### G-2 (medium): budget.py non-blocking lock → blocking lock

**Problem**: Windows uses `msvcrt.LK_NBLCK` which raises `IOError` immediately if another process holds the lock. This causes spurious failures under concurrent access instead of waiting.

**Fix**: Switch to `msvcrt.LK_LOCK` (blocking) with a timeout. Unix `fcntl.LOCK_EX` is already blocking.
...
def _file_lock(f, exclusive: bool = True) -> None:
    ...
    Timeout is handled by the threading lock (_lock) wrapping all callers.
    """
    import sys
    if sys.platform == "win32":
        import msvcrt
        # v17 G-2: LK_LOCK blocks until lock is available (was LK_NBLCK)
        msvcrt.locking(f.fileno(), msvcrt.LK_LOCK if exclusive else msvcrt.LK_UNLCK, 1)
    else:
        import fcntl
        fcntl.flock(f.fileno(), fcntl.LOCK_EX if exclusive else fcntl.LOCK_UN)
```

### G-3 (medium): Security policy documentation

**Problem**: The consent policy system (`ORCHESTRA_CONSENT_POLICY`) and directory allowlist configuration lack user-facing documentation.

**Fix**: Add a `docs/security-policy.md` to the implementation, created during Step 2 (directory creation). Content:

```markdown
# Orchestra Security Policy

## Context Guard

All content sent to external agents (Codex, Gemini) passes through `context_guard.py`.

### Layers

1. **Size limit**: Content truncated to 100,000 characters before scanning.
2. **Source origin check**: If `source_files` is not provided, consent policy applies.
3. **Directory allowlist**: Source files must be under allowed directories.
4. **File blocklist**: Files with sensitive extensions (.env, .pem, .key, etc.) are blocked.
5. **Secret scanning**: Regex patterns detect API keys, tokens, private keys, JWTs, connection strings.
6. **Policy enforcement**: Based on `ORCHESTRA_CONSENT_POLICY`.

### Consent Policy (`ORCHESTRA_CONSENT_POLICY`)

| Value | Behavior |
|-------|----------|
| `redact` (default) | Detected secrets are replaced with `[REDACTED]`, transmission continues. |
| `block` | Any secret finding blocks transmission entirely. |
| `require_allowlist` | `source_files` must be explicitly provided; unknown-origin content is blocked. |

Set via environment variable:
```bash
export ORCHESTRA_CONSENT_POLICY=block
```

Invalid values default to `redact` with an audit log warning.

### Directory Allowlist

Files sent to external agents must be under one of:
- `~/.claude/`
- `$CLAUDE_PROJECT_DIR` (set by `bootstrap.py` or manually)
- `$ORCHESTRA_ALLOWED_DIRS` (comma-separated list of additional directories)

```bash
export CLAUDE_PROJECT_DIR=/path/to/project
export ORCHESTRA_ALLOWED_DIRS=/path/to/shared,/path/to/data
```

If neither `CLAUDE_PROJECT_DIR` nor `ORCHESTRA_ALLOWED_DIRS` is set, project files will be blocked with an audit warning.

### Source Files Parameter

| `source_files` value | Interpretation |
|---------------------|---------------|
| `["file1.py", "file2.py"]` | Known origin: files are validated against allowlist and blocklist. |
| `None` | Unknown origin: consent policy determines behavior (see above). |
| `[]` (empty list) | Same as `None` — treated as unknown origin (v16 F-1). |

### Audit Log

All guard decisions are logged to `~/.claude/logs/context_guard_audit.jsonl` (non-interactive, best-effort).

### Budget Control

- Default token budget: 500,000 per session (`ORCHESTRA_TOKEN_BUDGET`)
- Default max concurrent agents: 1 (`ORCHESTRA_MAX_CONCURRENT`)
- Budget state: `~/.claude/logs/budget_session.json` (file-locked for inter-process safety)
```

### G-4 (low): `source_files=[]` vs unknown origin distinction in documentation

**Problem**: The semantic difference between `source_files=None` (caller didn't provide info) and `source_files=[]` (caller explicitly states no files) is not documented.

**Fix**: Addressed in G-3 documentation above. Both `None` and `[]` are treated identically as "unknown origin" per v16 F-1 design decision. The documentation table makes this explicit. Additionally, add inline docstring clarification in `guard_context()`:
...
def guard_context(content: str, source_files: list[str] = None) -> str:
    ...
    Args:
        ...
        source_files: List of source file paths for provenance tracking.
            ...
            - [] (empty list): Treated same as None (v16 F-1).
...
## v16 Changes (from v15 Codex Verify Review)

### Review F: v15 Codex Verify Issues (1 high + 2 medium + 1 low)

| # | Severity | v15 Issue | v16 Fix |
|---|----------|----------|---------|
| F-1 | **high** | `source_files=[]` bypasses `require_allowlist` because `if source_files:` is False for empty list | Changed all `if source_files:` checks to `if source_files is not None and len(source_files) > 0:` for allowlist/blocked-file checks. Unknown-origin handling now triggers on `source_files is None or len(source_files) == 0`. |
| F-2 | medium | `record_call()` and `release_slot()` both decrement `active_calls` → double-decrement | Removed `active_calls` decrement from `record_call()`. Only `release_slot()` decrements. |
| F-3 | medium | `cwd()` removed but `CLAUDE_PROJECT_DIR` not guaranteed → project files blocked | `bootstrap.py` now sets `CLAUDE_PROJECT_DIR` from settings.json or auto-detects. `_build_allowed_dirs()` emits warning if neither env var is set. |
| F-4 | low | `CONSENT_POLICY` resolved at import time, runtime changes ignored | Policy now resolved inside `guard_context()` on each call via `_get_consent_policy()`. Module-level constant removed. |

---

## v15 Changes (from v14 Codex Verify Review)

### Review E: v14 Codex Verify Issues (1 high + 2 medium + 2 low)

| # | Severity | v14 Issue | v15 Fix |
|---|----------|----------|---------|
| E-1 | **high** | Wrappers only accept `context: str`, cannot pass `source_files` to `guard_context()` → allowlist bypass under default `redact` policy | **Root cause fix**: `call_codex()`, `call_codex_safe()`, and gemini equivalents now accept `source_files: list[str] = None` parameter and pass it through to `guard_context()`. All orchestrator call sites updated to provide file provenance. |
| E-2 | medium | `cwd()` unconditionally added to `_ALLOWED_BASE_DIRS` → overly permissive if hooks run from broad dirs | `cwd()` no longer auto-added. Only `~/.claude`, `CLAUDE_PROJECT_DIR`, and explicit `ORCHESTRA_ALLOWED_DIRS` are included. |
| E-3 | medium | `reset_session()` / `get_summary()` still use unlocked `_load_state()` | Migrated to `_load_state_locked()`/`_save_state_locked()`. Old `_load_state()`/`_save_state()` removed entirely. |
| E-4 | low | `ORCHESTRA_CONSENT_POLICY` typo silently defaults to `redact` | Added `_validate_consent_policy()` that warns on invalid values and defaults to `redact` with log entry. |
| E-5 | low | `guard_context_report()` doesn't enforce policies → potential bypass if misused | Renamed to `_guard_context_report_internal()` (private). Added docstring: "For internal diagnostics only, NOT for outbound content." |

---

## v14 Changes (from v13 Codex Verify Review)

### Review D: v13 Codex Verify Issues (1 high + 3 medium + 2 low)

| # | Severity | v13 Issue | v14 Fix |
|---|----------|----------|---------|
| D-1 | **high** | Wrappers call `guard_context()` without `source_files` → allowlist enforcement bypassed | `guard_context()` redesigned: when `source_files` is None, content is treated as "unknown origin" and policy decides (block/redact). Wrappers updated to pass source info. Added `_ALLOWED_BASE_DIRS` expansion via `ORCHESTRA_ALLOWED_DIRS` env var for project roots. |
| D-2 | medium | `_ALLOWED_BASE_DIRS` only `~/.claude` → project files all blocked | `_ALLOWED_BASE_DIRS` now dynamically includes `CLAUDE_PROJECT_DIR` env var and `ORCHESTRA_ALLOWED_DIRS` (comma-separated). Defaults to `~/.claude` + cwd. |
| D-3 | medium | `budget.py` `_load_state_locked` unused, read-modify-write not atomic | All budget operations (`check_budget`, `acquire_slot`, `release_slot`, `record_call`) now use `_load_state_locked`/`_save_state_locked` for atomic read-modify-write. Old `_load_state`/`_save_state` removed. |
| D-4 | medium | Consent gate is audit-log only, no policy control | Added `ORCHESTRA_CONSENT_POLICY` env var: `block` (block on any finding), `redact` (redact and continue, default), `require_allowlist` (require explicit source_files). Policy enforced in `guard_context()`. |
| D-5 | low | `make_error_response` returns `confidence: 0` violating 1-10 range | Changed to `confidence: 1` |
| D-6 | low | Full regex scan before size limit → performance risk on large input | Size limit now applied BEFORE regex scan. Truncation happens first, then scan on truncated content. |
...
## New Files in v12 (updated with v17 changes)
...
### gemini_wrapper.py changes (A-2, A-4, E-1)

Same pattern as codex_wrapper:
- `call_gemini(mode, context, timeout, source_files=None)` — accepts `source_files` parameter (v15 E-1)
- `guard_context(prompt, source_files=source_files)` before stdin (v12 A-2 + v15 E-1)
- `validate_output()` after JSON parse (v12 A-4)
- `call_gemini_safe(mode, context, timeout, source_files=None)` — safe wrapper with budget/retry, propagates `source_files` (v15 E-1)
...
## Updated Security Policy (v12, v17 G-3 doc added)
...
11. **[v17 NEW] Security policy documentation**: `docs/security-policy.md` provides user-facing documentation for consent policy, directory allowlist, and source_files semantics
...
+-- __init__.py
+-- bootstrap.py           # Common init for all hooks/scripts (v16: sets CLAUDE_PROJECT_DIR)
+-- codex_wrapper.py       # Codex CLI wrapper (v15: +source_files propagation, +context_guard, +output_schemas, +retry)
+-- gemini_wrapper.py      # Gemini CLI wrapper (v15: +source_files propagation, +context_guard, +output_schemas)
+-- vault_sync.py          # TetsuyaSynapse sync (v12: +docs versioning, B-2 fix)
+-- path_utils.py          # Windows/Git Bash path normalization
+-- context_guard.py       # [NEW] Secret scanning, redaction, dir enforcement, audit log (v17: per-call dir resolution)
...
+-- budget.py              # [NEW] Token budget and concurrency control (v17: blocking file lock)
...
## Implementation Order (v12 updated, v17 additions)
...
4. **[NEW] Create context_guard.py** (v17: per-call `_build_allowed_dirs()`)
5. **[NEW] Create resilience.py**
6. **[NEW] Create output_schemas.py**
7. **[NEW] Create env_check.py**
8. **[NEW] Create budget.py** (v17: blocking file lock)
...
20. **[v17 NEW] Create docs/security-policy.md** (G-3)
21. Run env_check.py to generate capability report
22. Verification (all v11 tests + context_guard test + output validation test)

---

## Review Request (v17 - updated with Codex Verify H feedback)

v16 was APPROVED with 2 medium + 2 low remaining items. v17 addresses all 4 original items (G-1~G-4) plus 2 new items from v17 Codex Verify (H-1, H-3):

### G fixes (v16 remaining items):
1. **[Medium] `_ALLOWED_BASE_DIRS` import-time cache** (G-1): Removed module-level constant. Per-call resolution via `_build_allowed_dirs()`.
2. **[Medium] Non-blocking file lock on Windows** (G-2): `msvcrt.LK_NBLCK` → `msvcrt.LK_LOCK` (blocking).
3. **[Medium] Security policy documentation** (G-3): Added `docs/security-policy.md`.
4. **[Low] `source_files=[]` vs unknown origin** (G-4): Documented in code and policy doc.

### H fixes (v17 Codex Verify feedback):
5. **[Medium] Unknown-origin content bypass** (H-1): Added `ORCHESTRA_STRICT_ORIGIN=1` env var. When set, blocks all content without explicit `source_files`, regardless of consent policy. Documented in `docs/security-policy.md`.
6. **[Medium] `check_budget` exception bubble** (H-3): Wrapped `_load_state_locked()` in try/except, returns permissive fallback on failure. Consistent with "best-effort" budget tracking design.
7. **H-2 and H-4 were already addressed by G-1 and G-3 respectively.**

### I fixes (v17 Codex Verify re-review feedback):
8. **[Medium] Fallback secret leakage** (I-3): `fallback_to_orchestrator()` now applies `redact_secrets()` to `original_task` before including it in fallback responses. Falls back to truncation (50 chars) if redaction import fails.

All v12-v16 fixes are preserved. Please review and verify these v17 fixes are adequate.
