# Claude Code Orchestra Integration Plan v15
...
def _validate_consent_policy() -> str:
    """v15 E-4: Validate consent policy, warn on typo."""
...
CONSENT_POLICY = _validate_consent_policy()
...
def _build_allowed_dirs() -> list[Path]:
    ...
    # Use CLAUDE_PROJECT_DIR or ORCHESTRA_ALLOWED_DIRS for project roots
    return dirs
...
def guard_context(content: str, source_files: list[str] = None) -> str:
    ...
    # Step 2: Unknown origin policy (v14 D-1)
    if source_files is None:
        if CONSENT_POLICY == "require_allowlist":
        ...
        _audit_log("unknown_origin", f"No source_files provided, policy={CONSENT_POLICY}")

    # Step 3: Enforce allowed base directories (v13 C-1)
    if source_files:
    ...
    # Step 4: File allowlist check (blocked extensions/patterns)
    if source_files:
    ...
    if findings:
        _audit_log("secrets_found", f"{len(findings)} secrets detected, policy={CONSENT_POLICY}")
        if CONSENT_POLICY == "block":
            raise ContextGuardError(
...
v14 Codex Verify flagged 5 issues (1 high + 2 medium + 2 low). This v15 addresses all of them:

1. **[High] Wrapper source_files propagation** (E-1): `call_codex()`, `call_codex_safe()`, and gemini equivalents now accept `source_files: list[str] = None` parameter and pass it through to `guard_context()`. All call chains propagate file provenance end-to-end.
2. **[Medium] cwd() overly permissive** (E-2): `cwd()` removed from `_ALLOWED_BASE_DIRS`. Only `~/.claude`, `CLAUDE_PROJECT_DIR`, and explicit `ORCHESTRA_ALLOWED_DIRS` are included.
3. **[Medium] Unlocked budget functions** (E-3): `_load_state()` and `_save_state()` removed entirely. `reset_session()` and `get_summary()` migrated to `_load_state_locked()`/`_save_state_locked()`. All budget operations are now atomic with file locks.
4. **[Low] Policy typo silent default** (E-4): Added `_validate_consent_policy()` that validates against known values and logs warning on invalid input.
5. **[Low] guard_context_report bypass risk** (E-5): Renamed to `_guard_context_report_internal()` (private function) with explicit docstring: "For internal diagnostics only, NOT for outbound content."

All v11-v14 fixes are preserved. Please review and verify these v15 fixes are adequate.
