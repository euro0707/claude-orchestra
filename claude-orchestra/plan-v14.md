# Claude Code Orchestra Integration Plan v14
...
CONSENT_POLICY = os.environ.get("ORCHESTRA_CONSENT_POLICY", "redact")
...
v13 Codex Verify flagged 6 issues (1 high + 3 medium + 2 low). This v14 addresses all of them:

1. **[High] source_files bypass** (D-1): `guard_context()` redesigned — unknown origin handled by consent policy, wrappers must pass source info. `_ALLOWED_BASE_DIRS` dynamically expanded via env vars.
2. **[Medium] Allowed dirs too restrictive** (D-2): `_ALLOWED_BASE_DIRS` now includes `CLAUDE_PROJECT_DIR`, `ORCHESTRA_ALLOWED_DIRS` env var, and `cwd()` by default.
3. **[Medium] Budget read-modify-write not atomic** (D-3): All budget operations (`check_budget`, `acquire_slot`, `release_slot`, `record_call`) now use `_load_state_locked`/`_save_state_locked` for atomic file-locked read-modify-write. Old unlocked functions removed.
4. **[Medium] Consent gate audit-only** (D-4): Added `ORCHESTRA_CONSENT_POLICY` env var with three modes: `block`, `redact` (default), `require_allowlist`. Policy enforced in `guard_context()`.
5. **[Low] confidence:0 in error response** (D-5): Changed to `confidence: 1` (minimum valid value in 1-10 range).
6. **[Low] Performance: full scan before truncation** (D-6): Size limit now applied BEFORE regex scan. Truncation first, then scan on truncated content.

All v11, v12, and v13 fixes are preserved. Please review and verify these v14 fixes are adequate.
