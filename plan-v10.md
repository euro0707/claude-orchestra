...
# Claude Code Orchestra Integration Plan v10
...
> python -c "import subprocess, shutil; node=shutil.which('node'); r=subprocess.run([node, 'C:/Users/skyeu/AppData/Roaming/npm/node_modules/@openai/codex/bin/codex.js', '--version'], capture_output=True, text=True); print(r.stdout, r.stderr)"

Test 2: Does codex exec return stdout (prompt via stdin)?
> python -c "import subprocess, shutil; node=shutil.which('node'); r=subprocess.run([node, 'C:/Users/skyeu/AppData/Roaming/npm/node_modules/@openai/codex/bin/codex.js', 'exec', '--full-auto', '--json', '-'], input='What is 1+1?', capture_output=True, text=True, timeout=60); print('stdout:', repr(r.stdout[:500])); print('stderr:', repr(r.stderr[:500]))"

Test 3: Does -o temp file output work (Stage 2 validation)?
> python -c "import subprocess, shutil, tempfile, os; node=shutil.which('node'); out=os.path.join(tempfile.gettempdir(),'codex_test_out.md'); r=subprocess.run([node, 'C:/Users/skyeu/AppData/Roaming/npm/node_modules/@openai/codex/bin/codex.js', 'exec', '--full-auto', '-o', out, '-'], input='What is 1+1?', text=True, timeout=60); exists=os.path.exists(out); size=os.path.getsize(out) if exists else 0; print('file exists:', exists, 'size:', size); content=open(out).read()[:500] if exists and size>0 else ''; print('content:', repr(content))"
```
...
- `get_prompt_template()` — mode-based prompt generation
- `parse_json_from_output()` — regex JSON extraction (codex_wrapper currently only uses json.loads)
...
- `load_context()` — file-based context loading
...
### Medium Risk
- Hook execution performance: agent-router.py fires on every prompt. Add early-return conditions (skip short prompts).
